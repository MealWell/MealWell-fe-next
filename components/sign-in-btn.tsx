...
export async function SignInButton() {
	...
	return (
		<Link
			href={session?.session ? "/profile" : "/sign-in"}
		...
		>
			<Button className="gap-2  justify-between" variant="default">
				...
				<span>{session?.session ? "Profile" : "Sign In"}</span>
			</Button>
		</Link>
	);
}
...
export async function SignInFallback() {
	...
	return (
		<Link
			href={guessIsSignIn ? "/profile" : "/sign-in"}
			className="flex justify-center"
		>
			<Button className="gap-2  justify-between" variant="default">
				{!guessIsSignIn ? (
				...
				<span>{guessIsSignIn ? "Profile" : "Sign In"}</span>
			</Button>
		</Link>
	);
}
